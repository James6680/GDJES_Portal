...
<body>
    ...
    <button id="addEditButton" onclick="addEditButtonClick('add', this)">Add Subject</button>
    <input type="text" id="myInput" onkeyup="filter()" placeholder="Search subject or grade level..." title="Type in a name">
    <table id="myTable">
        <tr class="header">
    ...
    <div id="myModal" class="modal">
        ...
        <div class="modal-content">
            ...
            <form method="post" action="" id="CRUDData">
                    @csrf
                @method('post')
                <label id="subject_name-error" class="error" style="color: red;"></label><br>
                <label for="grade_level_id">Grade Level:</label>
                <input hidden value="" id="formAction" name="formAction">
                <input hidden value="" id="subject_id" name="subject_id">
    ...
    <script>
        ...
        function addEditButtonClick(mode, button) {
            ...
            if(mode === "edit"){
                ...
                const subjectName = tableRow.querySelector("td:first-child").textContent;
                const gradeLevelId = tableRow.querySelector("td:nth-child(2)").textContent;
                const subjectId = tableRow.querySelector("td:nth-child(3)").textContent;
          ...
          // Populate dropdown with list of regions
            $.getJSON(url, function(data) {
              ...
              // Add a cell for each column in the table.
                    row.append($('<td>').text(entry.subject_name));
        ...
        function submitForm(form){
            ...
            $.ajax({
                url: "{{ route('admin.CRUDSubject') }}",
                type: "POST",
                data: formData,
                success: function(response) {
                    // Handle successful response'
                    $("#subject_name-error").css("color", "green");
                    var mode = $('#formAction').val();
                    if(mode === "AddData")
                        $('#subject_name-error').text('Subject Inserted Successfully');
                    else if(mode === "EditData")
                        $('#subject_name-error').text('Subject Edited Successfully');
                    getTableData();
                },  
                error: function(error) {
                    // Handle error response
                    // Display the validation error messages
                    $("#subject_name-error").css("color", "red");
                    $('.error').text('');
